...
public:
    ...
    */
    ...
    /** Copies an (interleaved) STK buffer onto this one.
     This buffer's size will be changed to that of the other buffer.
     */
    AudioSampleBuffer& operator= (::stk::StkFrames& other) noexcept;
    
    /** Adds an STK (interleaved) buffer onto this one.
     This buffer's size will be changed to that of the other buffer.
     */
//    AudioSampleBuffer& operator+= (::stk::StkFrames& other) noexcept;
    
//    AudioSampleBuffer& copy(::stk::StkFrames& other, int destStart, int numSamples = 0) noexcept;
    ...
    */
    ...
    /** Adds samples from an (interleaved) STK buffer to this one.
     
     @param destStartSample      the start sample within this buffer's channel
     @param source               the source buffer to add from
     @param numSamples           optional number of samples to process (defaults to whole STK buffer)
     @param gainToApplyToSource  an optional gain to apply to the source samples before they are
                                 added to this buffer's samples
    ...
    void addFrom (int destStartSample,
