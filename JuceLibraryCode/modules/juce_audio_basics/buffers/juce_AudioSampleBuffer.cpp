...
AudioSampleBuffer& AudioSampleBuffer::operator= (::stk::StkFrames& other) noexcept
{
    setSize(other.channels(), other.frames());
    
    for (int i = 0; i < numChannels; ++i)
    {
        float* pSampleOut = channels[i];
        ::stk::StkFloat* pSampleIn = &other[i];
        for(int j = 0; j < size; j++)
        {
            *pSampleOut++ = *pSampleIn;
            pSampleIn += numChannels;
        }
    }
...
void AudioSampleBuffer::addFrom(int destStartSample,
                                ::stk::StkFrames& source,
                                int numSamples,
                                float gainToApplyToSource) noexcept
{
    if(numSamples == 0)
        numSamples = source.frames();
        
    for (int i = 0; i < numChannels; ++i)
    {
        float* pSampleOut = &channels[i][destStartSample];
        ::stk::StkFloat* pSampleIn = &source[i];
        for(int j = destStartSample, k = 0; j < size && k < numSamples; j++, k++)
        {
            *pSampleOut++ += *pSampleIn * gainToApplyToSource;
            pSampleIn += numChannels;
        }
    }
}
